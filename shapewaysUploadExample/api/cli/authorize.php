...
try {
    ...
    if ( array_key_exists('oauth_token', $info) &&
        ...
        echo "ERROR: failed to get a request token";
    }
} catch(OAuthException $exception){
  echo 'ERROR: failed to get a request token.  Exception: ' . $exception->getMessage();
}
...
try {
  ...
  echo 'ERROR: failed to get access token.  Exception: ' . $exception->getMessage();
}